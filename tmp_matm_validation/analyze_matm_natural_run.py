...
def main():
    ...
    def usable_id(v):
        if v is None or (isinstance(v,float) and math.isnan(v)):
            return None
        s=str(v).strip()
        return None if s.lower() in {"", "na", "n/a", "none", "null"} else s
    for idx,row in df.iterrows():
        r=row.to_dict(); uid=(usable_id(r.get("trajectory_id")) or usable_id(r.get("run_id")) or usable_id(r.get("episode_id")) or f"row_{idx}")
